...
def _train_block_model(
    ...
    # Determine feature columns — P29: ALWAYS load from canonical file
    feat_path = MODELS_DIR / "feature_names.json"
    if feat_path.exists():
        base_cols = json.loads(feat_path.read_text())
    else:
        try:
    ...
    feature_cols = [c for c in base_cols if c in feat_df.columns]
    missing_from_block = [c for c in base_cols if c not in feat_df.columns]
    if missing_from_block:
        logger.warning(
            "P29 Block [end=%s]: %d canonical features missing: %s",
            train_end_date, len(missing_from_block), missing_from_block,
        )
    ...
    # P29: Hard subset assertion — same count as main training
    logger.info(
        "P29 WF BLOCK COLS (%d): %s", len(feature_cols), sorted(feature_cols)
    )
